...
Interpretation: Looking at the two maps, we can say that there was an overall increase in the percentage of seats occupied by women in parliament between years 2006 and 2016 in the countries shown on the map. The percentage change looks most significant in Mexico and least significant in Chile. 
...
#Combining datasets 
parliament_map_2016<- full_join(women_in_parliament_map_2016, world_map, by = "Country")%>%
  mutate(Time=2016)

parliament_map_2006<- full_join(women_in_parliament_map_2006, world_map, by = "Country")%>%
  mutate(Time=2006)
...
p4 <- parliament_map %>%
  ggplot() +
  geom_polygon(aes(x = long, y = lat,fill = parliament_female_share_seats_all, group=group), color = "white")+
 coord_map(projection = "mercator",
            xlim = c(-200,0), ylim = c(-40,150)) +
   scale_fill_gradient(limits=c(0,50),"Percentage of Seats", low = "#E0F7FA",
  high = "#1A237E", na.value= 'grey' ) +
...
Interpretation: This plot zooms into countries in Asia (including Russia) and shows us data from years 2019 and 2010. We cannot see a significant change in percentage of managerial positions occupied by women between 2010 and 2019. 
...
Interpretation: These plots look at female share of seats on boards of the largest publicly listed companies in Europe between years 2010 and 2020. Overall, it looks like the percentage of seats increased for most of the countries with the exception of Turkey, Greece, Lithuana and Estonia. This data again supports the information from previous plots where we saw that the difference in time spent in paid work between men and women were significantly higher in Turkey (also quite high in Lithuana). Through this we can interpret that the share of females in boards of the largest publicly listed companies among countries might be lower in countries where men spend more time in paid work than women/ where women spend more time in unpaid work than men. 
