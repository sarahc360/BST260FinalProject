...
#total work time among men 
total_time_men <- total_time%>%
  filter (Sex %in% "Men")

#total work time among women 
total_time_women <- total_time%>%
  filter (Sex %in% "Women")

#merging the two datatsets 
total_time_combined <- merge(total_time_men,total_time_women, by= "Country")
total_time_combined<- total_time_combined%>%
 mutate(total_diff= total_time.y-total_time.x)

#creating new variable by converting minutes to hours per day 
total_time_combined<- total_time_combined%>%
  mutate (total_time_combined_hours=total_diff/24)

```

```{r}
#Difference in time spent in unpaid work in OECD countries in the latest year (not sure if this refers to 2020 or any year before so this is a limitation of the dataset)
...
```{r}
#Difference in time spent in paid work paid work in OECD countries in the latest year (not sure if this refers to 2020 or any year before; this is a limitation of the dataset)
...
```{r}
#Difference in time spent in total work in OECD countries in the latest year (not sure if this refers to 2020 or any year before so this is a limitation of the dataset)

#Not sure how to do this because some values are neg 
...
p5 <- managers_map %>% filter(Time==2019)%>%
  ...
  geom_text_repel(min.segment.length = Inf, data = managers_labels, aes(long, lat, label = Country), color = "black") +
   ggtitle("Percentage of Women in Managerial Positions Worldwide (2019)")+
 theme(panel.grid.major = element_blank(), 
    panel.background = element_blank(),
       axis.title = element_blank(), 
      axis.text = element_blank(),
...
p6 <- managers_map_asia %>% filter(Time==2019)%>%
  ...
  coord_fixed(1.3)+
   ggtitle("Percentage of Women in Managerial Positions in Asia (2019)")+
 theme(panel.grid.major = element_blank(), 
    panel.background = element_blank(),
       axis.title = element_blank(), 
      axis.text = element_blank(),
...
p7 <- managers_map_asia %>% filter(Time==2010)%>%
  ...
  coord_fixed(1.3)+
   ggtitle("Percentage of Women in Managerial Positions in Asia (2010)")+
 theme(panel.grid.major = element_blank(), 
    panel.background = element_blank(),
       axis.title = element_blank(), 
      axis.text = element_blank(),
