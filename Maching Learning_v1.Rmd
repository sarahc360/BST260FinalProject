...
Objective: Using data from the OECD (The Organisation for Economic Co-operation and Development) database, we plan to build a regression tree to predict proportion of time spent in unpaid work based on other demographic and employment metrics.
...
With the training set, we will fit a regression tree using the `tree` function to predict proportion of time spent in unpaid work based on other variables in the model data. To identify predictors, we selected variables with the following criteria: 

- Variables without missing data (i.e. no NAs)
- Variables that may be associated with proportion of time in unpaid work

Based on the criteria (see below for visualizations of predictor variables vs. outcome), we narrowed down the predictors to continent (`region`), male vs. female (`Sex`), percentage of individuals in the country who are in the labor force (`labour)`, and percentage of individuals who work part-time (`part_time`). We excluded paid_time, unpaid_time, and total_time variables because they are directly related to proportion of time spent in unpaid work and would not be informative in the regression tree. 

```{r, message=FALSE, warning=FALSE, fig.height=4}
## Plots to show association between proportion of time in unpaid work vs. predictors in model

## Scatter plots of unpaid proportion vs. continuous predictors
kf_model_data %>% 
  gather(predictor, value, c(labour, part_time)) %>% 
  ggplot(aes(x = value, y = prop_unpaid)) + 
  geom_point(color="Blue") + 
  facet_wrap(~ predictor, scales = 'free',
             labeller = 
               as_labeller(c("labour"="Labour Force Participation (%)", 
                             "part_time" = "Share of Part-Time Work (%)"))) + 
  xlab(NULL) + ylab("Proportion of Time in Unpaid Work")

## Box plots of unpaid proportion vs. categorical predictors
p_fm <- kf_model_data %>%
  ggplot(aes(x = Sex, y = prop_unpaid)) + 
  geom_boxplot(aes(fill=Sex)) +
  scale_x_discrete(labels=c("Male","Female")) +
  labs(x="Sex",y="Proportion of Time in Unpaid Work") +
...
p_region <- kf_model_data %>%
  ...
  scale_x_discrete(labels=c("Europe","North America","South America","Middle East","Oceania","Asia")) +
  labs(x="Region",y="Proportion of Time in Unpaid Work") +
...
We then used the `tree` function to fit our regression tree for the outcome on the predictors of interest.

```{r}
## Regression tree for proportion of unpaid work
...
We predicted **median unpaid work percentages among females to be ~30 percentage points higher than among men (~50-60% for women vs. ~25-35% for men)**. For females in countries with < 55% labor participation, the median outcome is predicted to be ~59%. For females in countries with labor participation greater than 55%, **share of part time** is the next variable to estimate the outcome. In this subgroup, the model predicts a higher median unpaid work proportion when the share of part-time is greater than ~27% compared to when the share is lower. **Overall, these partitions predict that females will have spent a greater proportion of their time in unpaid work compared to males.**

While the linear regression did not find any statistically significant predictors for proportion of time spent in unpaid work among females, the regression tree is an analysis with other covariates that could predict the unpaid proportion from the database. The model is limited by small sample size and potential discrepancies in "latest year" reporting by each country, but provides directional insight on variables that can predict the outcome of interest.


We can use the regression tree to make predictions for the test set and calculate the mean square error. 

```{r}
## Make predictions for proportion of unpaid time in test set data
...
Using the model to make predictions on the test set, we found that the MSE of the model is 88.9.



##### **Section III: Pruning**

Having 6 nodes in the regression tree is helpful as a nuanced analysis of which variables and thresholds create meaningful partitions in the data for predictions. Next, we want to see if pruning the tree will improve our MSE.
...
title("Pruned Regression Tree Predicting Proportion of Time in Unpaid Work")
...
It appears that **2 nodes yields the lowest minimum error**. When we pruned the tree to have 2 nodes, the tree is simplified, with female vs. male being the sole predictor of the outcome. While this **tree has a lower MSE at 77.5, it does not provide additional insights**, as we can tell from descriptive statistics that the proportion of time in unpaid work differs by sex. From `cv.tree` analysis, it seems that trees with 3, 4, or 5 nodes would only have an incremental improvement in MSE compared to the tree with 6 nodes (for example, model with four nodes has MSE of 86.0 vs. original MSE of 88.9). Therefore, we decided that **the regression tree with 6 nodes provides the most insightful predictions for the outcome of proportion of time spent in unpaid work.**
